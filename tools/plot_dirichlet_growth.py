...
def make_checkpoints(limit: int, count: int, first: int) -> np.ndarray:
    ...
    points = points[(points >= 1) & (points <= limit)]
    if points[-1] != limit:
...
def compute_track(
    ...
    checkpoint_indices = np.searchsorted(nonzero_n, checkpoints, side="right") - 1
    terms = signs * np.exp((-sigma - 1j * t) * logs)
...
def main() -> None:
    ...
    parser.add_argument("--checkpoints", type=int, default=220)
    ...
    mu = raw[1 : limit + 1]
    nonzero_n = np.nonzero(mu)[0].astype(np.int64) + 1
    signs = mu[nonzero_n - 1].astype(np.float64)
    logs = np.log(nonzero_n.astype(np.float64))
    ...
    for sigma in sigmas:
        for label, t, is_zero in targets:
            values = compute_track(nonzero_n, signs, logs, checkpoints, sigma, t)
            ...
            final_rows.append({
                ...
                "max_abs_over_final_abs": float(abs_values.max() / max(abs_values[-1], 1e-300)),
                "last_over_prev_decade": float(abs_values[-1] / max(abs_values[np.searchsorted(checkpoints, max(1, limit // 10))], 1e-300)),
            })
    ...
    if args.output_csv:
        ...
        with args.output_csv.open("w", newline="", encoding="utf-8") as csv_file:
            ...
            writer.writerow(["N", "sigma", "label", "is_zero", "t", "abs_D_N", "real_D_N", "imag_D_N"])
            for sigma in sigmas:
                for label, t, is_zero in targets:
    ...
    for axis, sigma in zip(axes, sigmas):
        for idx, (label, t, is_zero) in enumerate(targets):
            ...
            if is_zero:
                ...
                axis.plot(checkpoints, values, color=color, linewidth=1.0, alpha=0.9, label=f"{label}: t={t:.3f}")
            else:
                cidx = idx - len(gammas)
    ...
    axes[-1].set_xlabel("N")
    fig.suptitle("Growth of Mobius Dirichlet partial sums at zeta-zero ordinates", y=0.997)
    fig.tight_layout()
    ...
    print(f"limit={limit}")
    print(f"squarefree_terms={nonzero_n.size}")
    print("summary:")
